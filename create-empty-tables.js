...
fs.readdirSync(schemaPath).forEach((file) => { // eslint-disable-line no-sync
  if (file !== 'index.js') {
    ...
    const table = new Table(filename, schema, null, dynamoose);
    table.create((err) => {
      if(!err) {
        logger.info(`*** Table ${filename} has been created ***`);
      } else {
        ...
        table.delete((err) => {
          if (!err) {
            table.create((err) => {
              ...
              {
                logger.info(`*** Table ${filename} created failed -- ${err.message} ***`);
              }
            });
          } else {
            logger.info(`*** Delete exist Table ${filename} failed -- ${err.message} ***`);
          }
        })
      }
    });
